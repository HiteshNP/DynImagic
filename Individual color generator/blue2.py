...
if group_image is None:
    ...
    # Define HSV range for silver color
    lower_blue2 = np.array([90, 50, 50])
    upper_blue2 = np.array([130, 255, 255])

    # Create a mask for silver color
    blue2_mask = cv2.inRange(hsv_image, lower_blue2, upper_blue2)

    # Find contours in the mask
    contours, _ = cv2.findContours(blue2_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # Save the image with bounding boxes for silver saree
    cv2.imwrite('blue2_saree_with_boxes.jpg', group_image)

    # Extract the largest ROI based on bounding boxes for silver saree
    largest_roi = extract_largest_roi(group_image, bounding_boxes)

    # Save the largest ROI without the border for silver saree
    if largest_roi is not None:
        cv2.imwrite('largest_blue2_roi_without_border.jpg', largest_roi)
