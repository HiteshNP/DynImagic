...
if group_image is None:
    ...
    # Define lower and upper bounds for dark magenta color in HSV format
    lower_dark_magenta = np.array([140, 50, 50])
    upper_dark_magenta = np.array([160, 255, 150])

    # Create a mask for dark magenta color
    dark_magenta_mask = cv2.inRange(hsv_image, lower_dark_magenta, upper_dark_magenta)

    # Find contours in the mask
    contours, _ = cv2.findContours(dark_magenta_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # Save the image with bounding boxes for dark magenta color
    cv2.imwrite('dark_magenta_saree_with_boxes.jpg', group_image)
    ...
    if largest_roi is not None:
        cv2.imwrite('largest_dark_magenta_roi_without_border.jpg', largest_roi)
