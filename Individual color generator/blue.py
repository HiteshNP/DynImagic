...
if group_image is None:
    ...
    # Convert to HSV for light blue color
    ...
    # Create a mask for light blue color
    light_blue_mask = cv2.inRange(hsv_image, lower_light_blue, upper_light_blue)

    # Find contours in the mask
    contours, _ = cv2.findContours(light_blue_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # Save the image with bounding boxes for light blue color
    cv2.imwrite('blue_saree_with_boxes.jpg', group_image)

    # Extract the largest ROI based on bounding boxes for light blue color
    largest_roi = extract_largest_roi(group_image, bounding_boxes)

    # Save the largest ROI without the border for light blue color
    if largest_roi is not None:
        cv2.imwrite('largest_blue_roi_without_border.jpg', largest_roi)
