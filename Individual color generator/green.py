...
if group_image is None:
    ...
    # Save the image with bounding boxes for green color
    cv2.imwrite('green_saree_with_boxes.jpg', group_image)

    # Extract the largest ROI based on bounding boxes for green color
    largest_roi = extract_largest_roi(group_image, bounding_boxes)

    # Save the largest ROI without the border for green color
    if largest_roi is not None:
        cv2.imwrite('largest_green_roi_without_border.jpg', largest_roi)
