...
if group_image is None:
    ...
    # Define HSV lower and upper bounds for light pink color
    lower_light_pink = np.array([120, 20, 200])
    upper_light_pink = np.array([200, 255, 255])

    # Convert the image to HSV
    hsv_image = cv2.cvtColor(group_image, cv2.COLOR_BGR2HSV)

    # Create a mask for light pink color
    light_pink_mask = cv2.inRange(hsv_image, lower_light_pink, upper_light_pink)

    # Find contours in the light pink mask
    contours, _ = cv2.findContours(light_pink_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # Save the image with bounding boxes for light pink color
    cv2.imwrite('light_pink_saree_with_boxes.jpg', group_image)

    # Extract the largest ROI based on bounding boxes for light pink color
    largest_roi = extract_largest_roi(group_image, bounding_boxes)

    # Save the largest ROI without the border for light pink color
    if largest_roi is not None:
        cv2.imwrite('largest_light_pink_roi_without_border.jpg', largest_roi)
